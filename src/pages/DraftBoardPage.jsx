      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-[#8a95a8] text-sm">
            ...
            {anyAssigned
              ? "traded picks highlighted"
              : "awaiting draft positions"}
          </p>
        </div>
      ...
      {!anyAssigned ? (
        <div className="bg-[#111418] border border-[#2a3040] rounded-2xl px-5 py-12 text-center">
          ...
          <div className="text-sm text-[#8a95a8]">
            The commissioner needs to assign draft positions first.
          </div>
        </div>
      ) : (
        <>
                ...
                <thead>
                  <tr className="border-b border-[#2a3040] bg-[#0a0c10]">
                    ...
                    {Array.from({ length: 12 }, (_, i) => i + 1).map((pos) => (
                      <th
                        key={pos}
                        className="px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-[#8a95a8] min-w-[100px]"
                      >
                        <div>Pos {pos}</div>
                        <div
                          className="text-[#4a5568] font-normal truncate"
                          style={{ fontSize: "9px" }}
                        >
                      ...
                      <tr key={round} className="hover:bg-[#1a1f27]/50">
                        ...
                        {Array.from({ length: 12 }, (_, i) => i + 1).map(
                          (pos) => {
                            ...
                            const wasTraded =
                              slot.currentOwner !== originalOwner;
                  ...
                  >
                    ...
                    <div className="w-40 shrink-0">
                      ...
                      <div className="text-[10px] text-[#4a5568]">
                        Pos {draftPositions[team.rosterId] || "—"}
                      </div>
                    </div>
                    <div className="flex-1 flex flex-wrap gap-1">
                      {picks.length === 0 ? (
                        ...
                        picks.map((pick) => {
                          const isTraded =
                            pick.position !== draftPositions[team.rosterId];
                          return (
                            <span
                              key={pick.id}
