...
export default function CommissionerPage() {
  const { teams, draftPositions, setDraftPosition, trades, updateTradeStatus, getTeam, keepers, teamAssets } = useStore();
  const [saved, setSaved] = useState(false);

  // Check if position is already taken by another team
  const takenPositions = Object.entries(draftPositions).reduce((acc, [rid, pos]) => {
    acc[pos] = Number(rid);
    return acc;
  }, {});
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8a95a8] text-sm mt-0.5">Assign draft positions & manage the league</p>
        </div>
        {allAssigned && (
      ...
      <div className="bg-[#111418] border border-[#2a3040] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#2a3040] flex items-center justify-between">
          <h2 className="font-semibold text-white">Draft Position Assignment</h2>
          <p className="text-xs text-[#8a95a8]">Position determines all 8 rounds (e.g. pos 3 = 1.03, 2.03...)</p>
        </div>
        <div className="divide-y divide-[#2a3040]">
          {teams.map((team, idx) => {
            ...
            return (
              <div key={team.rosterId} className="flex items-center gap-4 px-5 py-3 hover:bg-[#1a1f27] transition-colors">
                {/* Avatar */}
                <div className="w-9 h-9 rounded-full bg-[#1a1f27] border border-[#2a3040] flex items-center justify-center overflow-hidden shrink-0">
                  {team.avatar
                    ...
                    : <span className="text-sm font-bold text-[#00e5a0]">{team.displayName[0]?.toUpperCase()}</span>
                  }
                </div>

                {/* Team info */}
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-white text-sm truncate">{team.teamName}</div>
                ...
                <div className="text-xs text-center hidden sm:block">
                  <div className={`font-semibold ${keeperCount(team.rosterId) > 0 ? 'text-[#00e5a0]' : 'text-[#4a5568]'}`}>
                ...
                {/* Asset count */}
                <div className="text-xs text-center hidden sm:block">
                  <div className={`font-semibold ${counts.picks > 0 ? 'text-[#4da6ff]' : 'text-[#4a5568]'}`}>
                    {counts.picks}
                  </div>
                ...
                <div className="shrink-0">
                  <select
                    value={assigned || ''}
                    onChange={e => handlePositionChange(team.rosterId, e.target.value)}
                  ...
                  >
                    ...
                    {POSITIONS.map(p => (
                      <option key={p} value={p}>
                        Position {p}
                        {takenPositions[p] && takenPositions[p] !== team.rosterId ? ' ⚠' : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {assigned && (
                  <div className="text-xs text-[#8a95a8] w-28 hidden md:block">
                    1.{String(assigned).padStart(2,'0')}, 2.{String(assigned).padStart(2,'0')}...
                  </div>
                )}
              </div>
            );
      ...
      <div className="bg-[#111418] border border-[#2a3040] rounded-2xl overflow-hidden">
        ...
        {pendingTrades.length === 0 ? (
          <div className="px-5 py-8 text-center text-[#4a5568] text-sm">No pending trades</div>
        ) : (
          <div className="divide-y divide-[#2a3040]">
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs font-semibold text-[#8a95a8] uppercase tracking-wider">Trade Proposal</span>
                        <span className="text-xs text-[#4a5568]">{new Date(trade.timestamp).toLocaleDateString()}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <div className="text-xs font-semibold text-[#00e5a0] mb-1">{fromTeam?.teamName} sends:</div>
                          <div className="space-y-0.5">
                            {trade.fromAssets.map(a => (
                              <div key={a.id} className="text-xs text-[#8a95a8] flex items-center gap-1.5">
                                <span className={`w-1.5 h-1.5 rounded-full ${a.type === 'pick' ? 'bg-[#4da6ff]' : 'bg-[#00e5a0]'}`} />
                        ...
                        <div>
                          <div className="text-xs font-semibold text-[#ff6b35] mb-1">{toTeam?.teamName} sends:</div>
                          <div className="space-y-0.5">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#2a3040]">
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-[#8a95a8]">Team</th>
                <th className="text-center px-3 py-3 text-xs font-semibold uppercase tracking-wider text-[#8a95a8]">Draft Pos</th>
                <th className="text-center px-3 py-3 text-xs font-semibold uppercase tracking-wider text-[#8a95a8]">Keepers</th>
                <th className="text-center px-3 py-3 text-xs font-semibold uppercase tracking-wider text-[#8a95a8]">Picks</th>
                <th className="text-center px-3 py-3 text-xs font-semibold uppercase tracking-wider text-[#8a95a8]">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#2a3040]">
              {teams.map(team => {
                ...
                return (
                  <tr key={team.rosterId} className="hover:bg-[#1a1f27] transition-colors">
                    <td className="px-5 py-3 font-medium text-white">{team.teamName}</td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <span className={kCount > 0 ? 'text-white' : 'text-[#4a5568]'}>{kCount}/6</span>
                    </td>
                    <td className="px-3 py-3 text-center">
                      <span className={counts.picks > 0 ? 'text-[#4da6ff]' : 'text-[#4a5568]'}>{counts.picks}</span>
                    </td>
                    <td className="px-3 py-3 text-center">
