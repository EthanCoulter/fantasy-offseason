            ...
            <div className="space-y-1.5">
              {mySaved.map(sv => {
                ...
                const when = sv.updatedAt
                  ...
                  : '';
                return (
                  // Mobile: stack name+meta on top, buttons wrap on a second
                  // row. Desktop (sm+): name | actions side-by-side. Without
                  // this the four action buttons squeezed the name down to
                  // a few characters on a phone.
                  <div
                    key={sv.id}
                    className="flex flex-col sm:flex-row sm:items-center gap-2 text-xs bg-[#0a0c10] border border-[#2a3040] rounded-lg px-3 py-2"
                  >
                    <div className="flex-1 min-w-0 sm:pr-2">
                      <div className="text-sm font-semibold text-white break-words">{sv.name}</div>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-1.5 sm:shrink-0">
                      <button
                        onClick={() => handleLoadSaved(sv)}
                        className="flex-1 sm:flex-none px-2.5 py-1 text-[11px] font-semibold text-[#4da6ff] border border-[#4da6ff]/30 rounded-lg hover:bg-[#4da6ff]/10"
                        title="Load this save onto the working board (replaces current picks)"
                      >View / Load</button>
                      <button
                        onClick={() => handleOverwriteSaved(sv)}
                        className="flex-1 sm:flex-none px-2.5 py-1 text-[11px] font-semibold text-[#00e5a0] border border-[#00e5a0]/30 rounded-lg hover:bg-[#00e5a0]/10"
                        title="Update this save with the current working board"
                      >Update</button>
                      <button
                        onClick={() => handleRenameSaved(sv)}
                        className="flex-1 sm:flex-none px-2.5 py-1 text-[11px] font-semibold text-[#8a95a8] border border-[#2a3040] rounded-lg hover:text-white"
                      >Rename</button>
                      <button
                        onClick={() => handleDeleteSaved(sv)}
                        className="flex-1 sm:flex-none px-2.5 py-1 text-[11px] font-semibold text-[#8a95a8] border border-[#2a3040] rounded-lg hover:text-red-400 hover:border-red-400/40"
                      >Delete</button>
                    </div>
                  </div>
                );
